...
def plot_baseline_table(df):
    ...
    plt.table(
        cellText=table_data,
        colLabels=['Test', 'IOPS', 'Mean Lat (μs)', 'P95 Lat (μs)', 'P99 Lat (μs)'],
        loc='center',
        cellLoc='center'
    )
...
def plot_blocksize_sweep(df, access_pattern):
    ...
    if access_pattern.lower() == 'rand':
        ...
        if (pattern_df['write_iops'] > 0).any():
            ax1.plot(range(len(pattern_df)), pattern_df['write_iops'], 'r-o', label='Write IOPS')
        ax1.set_ylabel('IOPS')
    else:
        if (pattern_df['read_bw_mb'] > 0).any():
            ax1.plot(range(len(pattern_df)), pattern_df['read_bw_mb'], 'b-o', label='Read MB/s')
        if (pattern_df['write_bw_mb'] > 0).any():
            ax1.plot(range(len(pattern_df)), pattern_df['write_bw_mb'], 'r-o', label='Write MB/s')
