#!/usr/bin/env python3
"""
Roofline Model Plot for 3D Point Stencil Benchmarks
===================================================

This script generates roofline plots for 3D stencil performance analysis.
...
def calculate_arithmetic_intensity(kernel_type):
    """
    Calculate arithmetic intensity for 3D stencil operation.
    
    For a typical 7-point stencil:
    - 6 additions (neighbors) + 1 multiply (center coefficient)
    - Operations: ~7 FLOP per point
    
    Memory traffic (assuming float32):
    - Read 7 points: 7 * 4 = 28 bytes
    - Write 1 result: 4 bytes
    Total: 32 bytes per point
    
    Arithmetic Intensity = FLOP / Bytes
    """
    if 'float32' in kernel_type or 'int32' in kernel_type:
        return 7.0 / 32.0  # 7 FLOP / 32 bytes = 0.219 FLOP/byte
    elif 'float64' in kernel_type:
        return 7.0 / 64.0  # 7 FLOP / 64 bytes = 0.109 FLOP/byte
    else:
        return 7.0 / 32.0  # Default to float32
...
def create_roofline_plot(df, output_dir='plots_output', title_suffix='', use_fma=True):
    ...
    for _, row in perf_df.iterrows():
        ...
        marker = 'o'  # default
        if 'contiguous' in kernel_name.lower():
            marker = 'o'  # circle for contiguous
        elif 'stride' in kernel_name.lower():
    ...
    plt.title(f'Roofline Model for 3D Stencil Benchmarks{title_suffix}{fma_suffix}\n'
    ...
    output_path = Path(output_dir) / f'stencil_roofline_model{title_suffix.lower().replace(" ", "_")}{fma_file_suffix}.png'
...
def create_detailed_roofline_by_type(df, output_dir='plots_output', use_fma=True):
    ...
    # Different access patterns by pattern type
    pattern_types = df['Pattern'].unique()
    for pattern in pattern_types:
        if pattern != 'contiguous':  # Already handled above
            pattern_data = df[df['Pattern'] == pattern]
            if not pattern_data.empty:
                create_roofline_plot(pattern_data, output_dir, f' - {pattern.title()} Access', use_fma)

def print_performance_summary(perf_df, use_fma=True):
    """Print a summary of performance efficiency."""
    print("\n" + "="*80)
    print("3D STENCIL ROOFLINE ANALYSIS SUMMARY")
    print("="*80)
    ...
    print(f"  3D Stencil float32: {calculate_arithmetic_intensity('float32'):.3f} FLOP/Byte")
    print(f"  3D Stencil float64: {calculate_arithmetic_intensity('float64'):.3f} FLOP/Byte")
...
def main():
    parser = argparse.ArgumentParser(description='Generate roofline plots for 3D stencil benchmarks')
    parser.add_argument('--input', '-i', default='stencil_benchmark_v1.csv', 
    ...
    try:
        ...
        print("Make sure you're running this script from the Project13dpointstencil directory.")
