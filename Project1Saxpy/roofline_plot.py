#!/usr/bin/env python3
"""
Roofline Model Plot for SAXPY Benchmarks
========================================

This script generates roofline plots for SAXPY performance analysis.
...
def calculate_arithmetic_intensity(kernel_type):
    """
    Calculate arithmetic intensity for SAXPY operation.
    SAXPY: Y = a*X + Y
    
    Operations: 2 FLOP per element (1 multiply + 1 add)
    Memory traffic: 
    - Read X: 4 bytes (float32) or 8 bytes (float64)
    - Read Y: 4 bytes (float32) or 8 bytes (float64)  
    - Write Y: 4 bytes (float32) or 8 bytes (float64)
    Total: 12 bytes (float32) or 24 bytes (float64)
    
    Arithmetic Intensity = FLOP / Bytes
    """
    if 'float32' in kernel_type or 'int32' in kernel_type:
        return 2.0 / 12.0  # 2 FLOP / 12 bytes = 0.167 FLOP/byte
    elif 'float64' in kernel_type:
        return 2.0 / 24.0  # 2 FLOP / 24 bytes = 0.083 FLOP/byte
    else:
        return 2.0 / 12.0  # Default to float32
...
def create_roofline_plot(df, output_dir='plots_output', title_suffix='', use_fma=True):
    ...
    for _, row in perf_df.iterrows():
        ...
        marker = 'o'  # default
        if 'stride' in kernel_name:
            marker = 's'  # square for strided access
        elif 'gather' in kernel_name:
            marker = '^'  # triangle for gather
        elif 'scatter' in kernel_name:
            marker = 'v'  # inverted triangle for scatter
    ...
    plt.title(f'Roofline Model for SAXPY Benchmarks{title_suffix}{fma_suffix}\n'
    ...
    output_path = Path(output_dir) / f'saxpy_roofline_model{title_suffix.lower().replace(" ", "_")}{fma_file_suffix}.png'
...
def create_detailed_roofline_by_type(df, output_dir='plots_output', use_fma=True):
    ...
    float32_contiguous = df[
        ...
        (~df['Kernel'].str.contains('stride|gather|scatter'))
    ]
    ...
    strided_data = df[df['Kernel'].str.contains('stride')]
    if not strided_data.empty:
        create_roofline_plot(strided_data, output_dir, ' - Strided Access', use_fma)
    
    # Gather/Scatter patterns
    gather_scatter_data = df[df['Kernel'].str.contains('gather|scatter')]
    if not gather_scatter_data.empty:
        create_roofline_plot(gather_scatter_data, output_dir, ' - Gather/Scatter', use_fma)

def print_performance_summary(perf_df, use_fma=True):
    """Print a summary of performance efficiency."""
    print("\n" + "="*80)
    print("SAXPY ROOFLINE ANALYSIS SUMMARY")
    print("="*80)
    ...
    print(f"  SAXPY float32: {calculate_arithmetic_intensity('float32'):.3f} FLOP/Byte")
    print(f"  SAXPY float64: {calculate_arithmetic_intensity('float64'):.3f} FLOP/Byte")
...
def main():
    parser = argparse.ArgumentParser(description='Generate roofline plots for SAXPY benchmarks')
    parser.add_argument('--input', '-i', default='saxpy_benchmark.csv', 
    ...
    try:
        ...
        print("Make sure you're running this script from the Project1Saxpy directory.")
