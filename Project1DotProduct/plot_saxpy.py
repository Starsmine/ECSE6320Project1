...
# ---------- Main processing ----------
def main(csv_path: str, out_dir: str, use_touched: bool = False):
    ...
    if "Pattern" in df.columns:
        df["Pattern"] = df["Pattern"].map(lambda v: normalize_pattern(v))
    else:
        df["Pattern"] = ""

    # Normalize Aligned column: accept values like 'aligned', 'misaligned', '1', '0', True/False
    if "Aligned" in df.columns:
        df["Aligned"] = df["Aligned"].apply(lambda x: 1 if str(x).strip().lower() in ("1","true","t","yes","y","aligned") else 0)
    else:
        df["Aligned"] = 1

    # Ensure Stride is numeric if present
    if "Stride" in df.columns:
        df["Stride"] = pd.to_numeric(df["Stride"], errors="coerce").fillna(0).astype(int)
    else:
        df["Stride"] = 0

    # By default Size_KB is raw buffer footprint (N * bytes). Optionally compute using a Touched heuristic.
    if use_touched:
        # compute touched as an estimate: for stride tests touched = floor(N/stride), for gather approximate m
        MAX_STRIDE_FOR_GATHER = 8
        def compute_touched(row):
            pattern = str(row.get("Pattern", "")).lower()
            n = int(row.get("N", 0))
            stride = int(row.get("Stride", 0)) if row.get("Stride", 0) is not None else 0
        ...
        group_cols = ["Kernel", "Type", "N", "Aligned", "Touched", "Size_KB", "Pattern"]
    else:
        # Compute an effective "touched" element count so stride/gather tests
        # are plotted on the same working-set x-axis as base contiguous runs.
        # For contiguous tests touched == N. For strided tests we estimate touched = floor(N / Stride).
        # For gather tests buffer footprint was logged as buf_elems = m * max_stride + 1
        # so approximate touched = floor((N-1) / max_stride). The benchmark uses max_stride=8.
        MAX_STRIDE_FOR_GATHER = 8
        def compute_touched(row):
            pattern = str(row.get("Pattern", "")).lower()
            try:
            ...
            if "stride" in pattern:
                ...
                return max(1, n // max(1, stride))
            if "gather" in pattern:
                # estimate m from buf_elems (buf_elems ~= m * max_stride + 1)
                if n <= 1:
                    return 1
                return max(1, (n - 1) // MAX_STRIDE_FOR_GATHER)
            # default: contiguous or other patterns
            return max(1, n)

        df["Touched"] = df.apply(compute_touched, axis=1)
        # Size_KB is now working-set size in KB for plotting (touched elements * bytes per element)
        df["Size_KB"] = df["Touched"] * df["Type"].map(BYTE_WIDTH) / 1024.0
    ...
    # Determine group from Kernel name and Pattern (ensure scatter isn't lumped into base)
    def group_from_row(r):
        kn = str(r.get("Kernel", "")).lower()
        pat = str(r.get("Pattern", "")).lower()
        if "scatter" in kn or "scatter" in pat:
            return "scatter"
        if "gather" in kn or "gather" in pat:
            return "gather"
        if "stride" in kn or "stride" in pat:
            return "stride"
        return "base"

    summary["Group"] = summary.apply(group_from_row, axis=1)
    ...
    if not scalar_mask.any():
        ...
        # join baseline on Type, Touched, Aligned so stride/gather runs compare with scalar of same touched elements
        baseline = summary[scalar_mask].set_index(["Type", "Touched", "Aligned"])[["central"]].rename(columns={"central": "scalar_central"})
        merged = summary.merge(baseline, how="left", left_on=["Type", "Touched", "Aligned"], right_index=True)
        ...
        speedup_df["Group"] = speedup_df.apply(group_from_row, axis=1)
...
if __name__ == "__main__":
    ...
    p.add_argument("--use-touched", action="store_true", help="Compute Size_KB from an estimated 'touched' element count for stride/gather tests")
    args = p.parse_args()
    main(args.csv, args.out, args.use_touched)