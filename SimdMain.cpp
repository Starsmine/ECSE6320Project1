                   ...
                   size_t stride)
{
    ...
    f << kernel << "," << dtype << "," << n << "," << trial << "," << time_ms
      << "," << gflops << "," << y0 << "," << (aligned ? "1" : "0")
      << "," << pattern << "," << stride << "\n";
}
                             ...
                             int trials = 10)
{
    ...
    for (size_t m : ms) {
        ...
        std::string g_label = std::string(kernel_label) + " gather";
        double flops_per_trial_g = 2.0 * double(m);

        for (int t = 0; t < trials; ++t) {
            // reset y for the touched indices
            ...
            std::cout << g_label << " | N = " << buf_elems << (!use_aligned_loads ? " (misaligned)" : "") << ":\n";
            std::cout << "  Time: " << time_ms << " ms\n";
            std::cout << "  GFLOP/s: " << gflops << "\n";
            std::cout << "  Gathered" << "\n";
            std::cout << "  y[0] = " << y_base[idx[0]] << "\n\n";
            ...
            log_per_trial<T>("saxpy_benchmark.csv",
                            ...
                            use_aligned_loads,
                            "gather",
                            0); // stride 0 meaning N/A for gather or use 1 for touched spacing if desired
